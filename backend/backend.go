...
func questionGenerator(id string, player int, questPos int, real bool) {
	...
	playerAnswerPair := "p"+strconv.Itoa(player)+"_a"+strconv.Itoa(questPos)
	...
	queryReal := fmt.Sprintf("UPDATE sessions SET %s=%s, %s=\"\", %s=$1 WHERE id=$2", playerRealPair, strconv.Itoa(questPos), playerAnswerPair, playerQuestionPair)
	_, err = db.Exec(
		context.Background(),
		query, questStr, id)
	if err != nil {	log.Fatal(err) }
...
func player2Connect(c *gin.Context) {
	...
	for (p1_real==p1_rand1 || p1_real==p1_rand2) { p1_real=rand.Intn(2)+1 }
	p2_rand1 := rand.Intn(2)+1
	p2_rand2 := p2_rand1
	p2_real := p2_rand1
	for (p2_rand1==p2_rand2) { p2_rand2=rand.Intn(2)+1 }
	for (p2_real==p2_rand1 || p2_real==p2_rand2) { p2_real=rand.Intn(2)+1 }
